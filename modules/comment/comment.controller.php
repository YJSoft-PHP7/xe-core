...
class commentController extends comment
{
	...
	function insertComment($obj, $manual_inserted = FALSE)
	{
		...
		if(!$document_srl)
		{
			return new XEObject(-1, 'msg_invalid_document');
		}
	...
	function deleteComment($comment_srl, $is_admin = FALSE, $isMoveToTrash = FALSE)
	{
		...
		if(count($childs) > 0)
		{
			...
			if(!$deleteAllComment)
			{
				return new XEObject(-1, 'fail_to_delete_have_children');
			}
			else
			{
				foreach($childs as $val)
	...
	function updateVotedCount($comment_srl, $point = 1)
	{
		...
		// Return the result
		$output = new XEObject(0, $success_message);
		if($point > 0)
		{
