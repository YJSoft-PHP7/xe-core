...
class moduleAdminController extends module
{
	...
	function procModuleAdminCopyModule($args = NULL)
	{
		...
		for($i=1;$i<=10;$i++)
		{
			...
			if(!$mid) continue;
			if(!preg_match("/^[a-zA-Z]([a-zA-Z0-9_]*)$/i", $mid)) return new XEObject(-1, 'msg_limit_mid');
	...
	function procModuleAdminInsertGrant()
	{
		...
		$module_info = $oModuleModel->getModuleInfoByModuleSrl($module_srl, $columnList);
		if(!$module_info) return new XEObject(-1,'msg_invalid_request');
		// Register Admin ID
	...
	function procModuleAdminModuleSetup()
	{
		...
		if(!$vars->module_srls) return new XEObject(-1,'msg_invalid_request');

		$module_srls = explode(',',$vars->module_srls);
		if(count($module_srls) < 1) return new XEObject(-1,'msg_invalid_request');
	...
	function procModuleAdminModuleGrantSetup()
	{
		...
		if(!$module_srls) return new XEObject(-1,'msg_invalid_request');

		$modules = explode(',',$module_srls);
		if(count($modules) < 1) return new XEObject(-1,'msg_invalid_request');
	...
	function procModuleAdminInsertLang()
	{
		...
		if(!$args->name) return new XEObject(-1,'msg_invalid_request');
