...
class trashAdminController extends trash
{
	...
	function procTrashAdminEmptyTrash()
	{
		...
		if(!$output->toBool()) return new XEObject(-1, $output->message);

		if(!$this->_emptyTrash($trashSrls)) return new XEObject(-1, $lang->fail_empty);
	...
	function _relationDataDelete($isAll, &$trashSrls)
	{
		...
		if(is_array($output->data))
		{
			foreach($output->data as $oTrashVO)
			{
				...
				if(!is_dir(FileHandler::getRealPath($classPath))) return new XEObject(-1, 'not exist restore module directory');

				$classFile = sprintf('%s%s.admin.controller.php', $classPath, $oTrashVO->getOriginModule());
				$classFile = FileHandler::getRealPath($classFile);
				if(!file_exists($classFile)) return new XEObject(-1, 'not exist restore module class file');

				$oAdminController = getAdminController($oTrashVO->getOriginModule());
				if(!method_exists($oAdminController, 'emptyTrash')) return new XEObject(-1, 'not exist restore method in module class file');

				$output2 = $oAdminController->emptyTrash($oTrashVO->getSerializedObject());
				if(!$output2->toBool()) return new XEObject(-1, $output2->message);
			}
		}
		return new XEObject(0, $lang->success_deleted);
	}
	...
	function procTrashAdminRestore()
	{
		...
		if(is_array($trashSrlList))
		{
			...
			foreach($trashSrlList as $value)
			{
				...
				if(!method_exists($oAdminController, 'restoreTrash')) return new XEObject(-1, 'not exist restore method in module class file');
			...
			if(!$this->_emptyTrash($trashSrlList)) {
				$oDB->rollback();
				return new XEObject(-1, $lang->fail_empty);
			}
			$oDB->commit();
		}
