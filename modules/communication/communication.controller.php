...
class communicationController extends communication
{
	 ...
	 * @return void|XEObject (success : void, fail : Object)
	 */
	function procCommunicationUpdateAllowMessage()
	{
		if(!Context::get('is_logged'))
		{
			return new XEObject(-1, 'msg_not_logged');
		}
	...
	function procCommunicationSendMessage()
	{
		...
		if(!$receiver_srl)
		{
			return new XEObject(-1, 'msg_not_exists_member');
		}

		$title = trim(Context::get('title'));
		if(!$title)
		{
			return new XEObject(-1, 'msg_title_is_null');
		}

		$content = trim(Context::get('content'));
		if(!$content)
		{
			return new XEObject(-1, 'msg_content_is_null');
		}
		...
		if($logged_info->is_admin != 'Y')
		{
			if($receiver_member_info->allow_message == 'F')
			{
				...
				{
					return new XEObject(-1, 'msg_allow_message_to_friend');
				}
			}
			else if($receiver_member_info->allow_message == 'N')
			{
				return new XEObject(-1, 'msg_disallow_message');
			}
		}
	...
	function sendMessage($sender_srl, $receiver_srl, $title, $content, $sender_log = TRUE)
	{
		...
		return new XEObject(0, 'success_sended');
	}
	...
	function procCommunicationDeleteMessages()
	{
		...
		{
			return new XEObject(-1, 'msg_cart_is_null');
		}
