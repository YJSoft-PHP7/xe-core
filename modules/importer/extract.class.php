...
class extract
{
	...
	function openFile()
	{
		...
		{
			if(!file_exists($this->filename)) return new XEObject(-1,'msg_no_xml_file');
		...
		else
		{
			...
			if(!$this->fd) return new XEObject(-1,'msg_no_xml_file');
			...
			if(preg_match('/404 Not Found/i',$buff)) return new XEObject(-1,'msg_no_xml_file');
		}
