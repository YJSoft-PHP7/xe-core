...
class spamfilterModel extends spamfilter
{
	...
	function isDeniedIP()
	{
		...
		if(!count($ip_list)) return new XEObject();

		$count = count($ip_list);
		for($i=0;$i<$count;$i++)
		{
			$ip = str_replace('.', '\.', str_replace('*','(25[0-5]|2[0-4][0-9]|[01]?[0-9][0-9]?)',$ip_list[$i]->ipaddress));
			if(preg_match('/^'.$ip.'$/', $ipaddress, $matches)) return new XEObject(-1,'msg_alert_registered_denied_ip');
		}
	...
	function isDeniedWord($text)
	{
		...
		if(!count($word_list)) return new XEObject();
		...
		for($i=0;$i<$count;$i++)
		{
			...
			if(preg_match('/'.preg_quote($word,'/').'/is', $text))
			{
				...
				return new XEObject(-1,sprintf(Context::getLang('msg_alert_denied_word'), $word));
			}
		}
	...
	function checkLimited($isMessage = FALSE)
	{
		...
		if($config->limits != 'Y') return new XEObject();
		$limit_count = '3';
		$interval = '10';
		...
		if($count>=$limit_count)
		{
			...
			return new XEObject(-1, 'msg_alert_registered_denied_ip');
		}
	...
	function isInsertedTrackback($document_srl)
	{
		...
		if($count>0) return new XEObject(-1, 'msg_alert_trackback_denied');
