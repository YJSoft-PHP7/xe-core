...
class spamfilterController extends spamfilter
{
	...
	{
		if($_SESSION['avoid_log']) return new XEObject();
		...
		if($is_logged)
		{
			if($logged_info->is_admin == 'Y') return new XEObject();
			if($grant->manager) return new XEObject();
		}
	...
	function triggerInsertTrackback(&$obj)
	{
		...
		if($obj->title == $obj->excerpt)
		{
			...
			return new XEObject(-1,'msg_alert_trackback_denied');
		}
	...
	function insertIP($ipaddress_list, $description = null)
	{
		...
		if(!preg_match($regExr,$ipaddress_list)) return new XEObject(-1, 'msg_invalid');
