...
class ModuleInstaller
{
	...
	function uninstallModule()
	{
		...
		if(is_subclass_of($output, 'XEObject') && !$output->toBool())
		{
			return $output;
		}
...
class SFTPModuleInstaller extends ModuleInstaller
{
	 ...
	 * Constructor
	 *
	 * @param XEObject $package Package information
	 * @return void
	 */
	...
	function _connect()
	{
		...
		{
			return new XEObject(-1, 'msg_sftp_not_supported');
		}
	...
	function _removeFile($path)
	{
		...
		{
			return new XEObject(-1, sprintf(Context::getLang('msg_delete_file_failed'), $path));
		}
		return new XEObject();
	}
	...
	function _removeDir_real($path)
	{
		...
		{
			return new XEObject(-1, sprintf(Context::getLang('msg_delete_dir_failed'), $path));
		}
		return new XEObject();
	}
	...
	function _copyDir(&$file_list)
	{
		...
		{
			return new XEObject(-1, 'msg_ftp_password_input');
		}
...
class PHPFTPModuleInstaller extends ModuleInstaller
{
	...
	function _removeFile($path)
	{
		...
		{
			return new XEObject(-1, "failed to delete file " . $path);
		}
		return new XEObject();
	}
	...
	function _removeDir_real($path)
	{
		...
		{
			return new XEObject(-1, "failed to delete directory " . $path);
		}
		return new XEObject();
	}
			...
			foreach($file_list as $k => $file)
			{
				...
				for($i = 0; $i < count($path_list); $i++)
				{
					...
					if(!file_exists(FileHandler::getRealPath($real_path)))
					{
						...
						{
							return new XEObject(-1, "msg_make_directory_failed");
						}
						...
						if(strtoupper(substr(PHP_OS, 0, 3)) !== 'WIN')
						{
							if(function_exists('ftp_chmod'))
							{
								...
								{
									return new XEObject(-1, "msg_permission_adjust_failed");
								}
							}
							else
							{
				...
				{
					return new XEObject(-1, "msg_ftp_upload_failed");
				}
			}
		}
		$this->_close();
