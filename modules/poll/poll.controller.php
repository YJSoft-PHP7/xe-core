...
class pollController extends poll
{
	...
	function procPollInsert()
	{
		...
		if(!count($args->poll)) return new XEObject(-1, 'cmd_null_item');
	...
	function procPoll()
	{
		...
		if(!count($item_srls)) return new XEObject(-1, 'msg_check_poll_item');
		// Make sure is the poll has already been taken
		$oPollModel = getModel('poll');
		if($oPollModel->isPolled($poll_srl)) return new XEObject(-1, 'msg_already_poll');
	...
	function triggerDeleteDocumentPoll(&$obj)
	{
		...
		if(!$poll_srl) return new XEObject();
